...
export function requireProviderConfig(connector: OAuthConnector, provider: string): OAuthProviderConfig {
  ...
  if (!config) {
    throw new ConfiguredOAuthError(`OAuth provider "${provider}" is not available. Configure its credentials in Settings first.`);
  }
  if (!config.enabled) {
    throw new ConfiguredOAuthError(`OAuth provider "${provider}" is disabled. Enable it in Settings first.`);
  }
  if (!config.clientId || !config.clientSecret) {
    throw new ConfiguredOAuthError(`OAuth provider "${provider}" is not configured. Set its Client ID and Client Secret in Settings.`);
  }
  return config;
}
