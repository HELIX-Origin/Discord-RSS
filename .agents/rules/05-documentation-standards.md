...
4. **No Uncommitted Secret Documentation**: Documentation must reference **GitHub Secrets** using the `{SERVICE_NAME}_WEBHOOK_URL_{###}` naming convention (e.g., `DISCORD_WEBHOOK_URL_001`). Never include real webhook URLs, tokens, or site URLs in docs unless they are public demonstration URLs.
