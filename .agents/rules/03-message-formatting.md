...
2. **No Hardcoded URLs**: Webhook URLs and feed URLs must be loaded exclusively from `.env` using the naming pattern `{SERVICE_NAME}_WEBHOOK_URL_{###}` (e.g., `DISCOHOOK_WEBHOOK_URL_001`, `SITE_STATUS_WEBHOOK_URL_001`, `CUSTOM_SERVICE_WEBHOOK_URL_002`) or `{SOURCE}_RSS_URL_{###}` (e.g., `FORUM_RSS_URL_001`, `SITE_RSS_URL_001`). They must never appear as `.env` examples, hardcoded strings, or literal values in source.
...
5. **Optional Discohook Formatting**: Discohook-style embed formatting is permitted optionally (`.agents/skills/discohook.md`). If used, it must remain within the Python script payload construction (`build_discord_message`) and never rely on hardcoded webhook URLs or `.env` files.
