...
## Goals
Centralize all Discord embed construction within Python scripts using GitHub Secrets webhook naming (`{SERVICE_NAME} WEBHOOK URL {###}`).
...
- [x] Script loads webhooks from GitHub Secrets using `{SERVICE_NAME} WEBHOOK URL {###}` pattern
