...
**HELIX RSS** — a self-hosted, multi-user RSS/Atom-to-Discord service built entirely in TypeScript ESM. Native `http`, `node:sqlite`, built-in dashboard, Discord bot + API; posts directly to Discord webhooks. Runtime deps: none except optional `redis` for cross-instance coordination.
...
- `typescript-architecture.md` (Rule 02) — `src/` layout (`index.ts`, `server.ts`, `config.ts`, `app.ts`, `auth/`, `bot/`, `db/`, `feed/`, `http/`, `oauth/`, `scheduler/`, `state/`, `status/`, `webhook/`); `tsconfig.json`; `package.json`.
- `message-formatting.md` (Rule 03) — Discord embeds built in `src/webhook/discord.ts`; webhooks are per-user rows in SQLite (dashboard-managed, not env vars); `.env` uses clean standard env vars.
...
- `documentation-standards.md` (Rule 05) — `docs/README.md` index (when present), `.env.example` reference, agent documentation sync.
